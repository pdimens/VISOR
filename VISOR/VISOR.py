...
def main():
	...
	parser_linkedreads = subparsers.add_parser('XENIA', help='10X gENomics sImulAtor. Simulate linked-read FASTQ using regions specified in standard BED format')

	required = parser_linkedreads.add_argument_group('Required I/O arguments')

	required.add_argument('-s','--sample', help='one folder containing FASTA haplotypes such as those generated by HACk', metavar='FOLDER', required=True)
	required.add_argument('-b','--bedfile', help='one or more regions to simulate in standard BED format', metavar='BED', required=True)
	required.add_argument('-o','--output', help='output folder with for FASTQ files', metavar='FOLDER', required=True)
	required.add_argument('-O','--output-format', help='output format for FASTQ files [10x] {choices: 10x, stlfr, haplotagging, tellseq}', default="10x", metavar='', choices = ["10x", "stlfr", "haplotagging", "tellseq"])
	...
	molecules.add_argument('--barcode-type', help='Type of linked-read barcodes used for --barcodes [10x] {choices: 10x, stlfr, haplotagging, tellseq}', metavar='', default = "10x", choices = ["10x", "stlfr", "haplotagging", "tellseq"])
